...
Alur Utama:
Aktor memasukkan username, password, dan memilih role.
Sistem memvalidasi kredensial.
Sistem menampilkan dashboard (user: pemesanan; admin: daftar pesanan).
...
Alur Utama:
User memasukkan durasi (jam).
Sistem menghitung harga (misalnya, Rp10.000/jam).
...
Post-kondisi: Pesanan disimpan, harga dihitung, dan dapat dilihat admin.
Alur Utama:
User memilih menu (misalnya, Nasi Goreng - Rp15.000).
Sistem menghitung harga berdasarkan menu.
Sistem menyimpan pesanan.
Sistem menampilkan konfirmasi.
...
Alur Utama:
User mengakses halaman total harga.
Sistem menghitung total (billing game + makanan).
Sistem menampilkan total harga.
...
Sistem menampilkan daftar pesanan dengan detail (jenis, deskripsi, waktu, harga).
...
Sistem mengirim notifikasi ke dashboard admin (via WebSocket untuk produksi).
...
Alur Utama:
Admin mengakses halaman manajemen stok.
Admin memilih aksi (tambah, edit, hapus).
Sistem memperbarui daftar menu.
Sistem menampilkan konfirmasi.
...
Alur Utama:
Aktor menekan tombol logout.
Sistem mengakhiri sesi dan menampilkan halaman login.
